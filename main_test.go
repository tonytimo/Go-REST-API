...
func readTestData(t *testing.T, name string) []byte {
	t.Helper()
	content, err := os.ReadFile("./go-rest-demo/testdata/" + name)
	if err != nil {
...
func TestRecipesHandlerCRUD_Integration(t *testing.T) {
	...
	hamAndCheese := readTestData(t, "ham_and_cheese_recipe")
	hamAndCheeseReader := bytes.NewReader(hamAndCheese)

	hamAndCheeseWithButter := readTestData(t, "ham_and_cheese_with_butter_recipe")
	...
	assert.Contains(t, updatedHamAndCheese.Ingredients, recipes.Ingredients{Name: "butter"})
